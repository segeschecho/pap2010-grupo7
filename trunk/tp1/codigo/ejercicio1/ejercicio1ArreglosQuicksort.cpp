...
{
    char* pivot = palabras[ desde ];
    int i = desde;
    int j = hasta;

    while( i < j )
    {
        while( i < j && strcmp( pivot, palabras[ j ] ) <= 0 )
        {
            j--;
        }

        while( i < j && strcmp( palabras[ i ], pivot ) <= 0 )
        {
            i++;
        }
    ...
    char temp[ MAX_SIZE_PALABRA + 1 ];
    strcpy( temp, palabras[ i ] );
    strcpy( palabras[ i ], palabras[ desde ] );
    strcpy( palabras[ desde ], temp );

    return i;
}

void sort( int desde, int hasta )
{
    if ( desde < hasta )
    {
        int indicePivot = particionar( desde, hasta );
        sort( desde, indicePivot - 1 );
        sort( indicePivot + 1, hasta );
    }
}
...
int main()
{
    int cantPalabras = 0;
/*
    ifstream file ( "test1", ios_base::in );
